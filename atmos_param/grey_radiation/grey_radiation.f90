...
real    :: del_sw          = 0.0
real    :: check_ir_tau_eq       = 4.0
real    :: ir_tau_pole     = 4.0
real    :: atm_abs         = 0.2
real    :: sw_diff         = 0.0

real 	:: amp		   = 0.0	! ozone hole experiment
real	:: lat0		   = -90.0
real	:: lat1 	   = 90.0
real	:: pref   	   = 0.0
real	:: preswidth	   = 100.e2
real	:: latwidth	   = 20.0	! end ozone hole experiment

!real 	:: alpha_optdep	   = 1.0	! scale optical depth
...
namelist/grey_radiation_nml/ solar_constant, del_sol, &
           check_ir_tau_eq, ir_tau_pole, atm_abs, sw_diff, &
	   amp, lat0, lat1, pref, preswidth, latwidth, &
           long_pert, del_long, &
...
real, dimension(size(tdt,1), size(tdt,2), size(tdt,3))	:: Gaussian		
...
tau_0 = check_ir_tau_eq +(ir_tau_pole - check_ir_tau_eq)*ss*ss
...
! modif df 1-23-04: changing profile of IR absorber
   tau(:,k)       = tau_0(:) * (linear_tau * p_half(1,:,k)/p00 + (1.0 - linear_tau) &
       * (p_half(1,:,k)/p00)**4) ! INCLUDED SCALING FACTOR ALPHA
...
!ozone hole experiment			

do j = 1, size(tdt,2)			
do k = 1, size(tdt,3)

	Gaussian(:,j,k) = (exp(-((rad_to_deg*lat(:,j)-lat0)**2)/(2.*(latwidth**2))) &
			+ exp(-((rad_to_deg*lat(:,j)-lat1)**2)/(2.*(latwidth**2)))) &
			* exp(-((p_half(:,j,k)-pref)**2)/(2.*(preswidth**2)))
...
do k = 1, size(tdt,3)
do j = 1, size(tdt,2)
	tdt(:,j,k) = tdt(:,j,k) + (amp/86400.)*Gaussian(:,j,k)		
end do
end do

!end ozone hole experiment
